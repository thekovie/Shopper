...
const FormInput = React.forwardRef<
  ...
  FormItemProps<typeof Input, string | number> & { allowNegatives?: boolean }
>(
  (
    { label, description, onChange, value, allowNegatives = false, ...props },
    ref,
  ) => {
    ...
    const handleChangeText = (text: string) => {
      // Regex for allowed characters (digits and optionally a minus sign)
      const regex = allowNegatives ? /^-?\d*$/ : /^\d*$/;

      // If the input matches the regex (numbers only)
      if (regex.test(text)) {
        const parsedValue = text === "" ? 0 : Number(text); // Default to 0 if empty
    ...
    return (
      <FormItem>
        ...
        <Input
          ref={inputRef}
          value={typeof value === "number" ? value.toString() : value} // Convert number to string for TextInput
          ...
          onChangeText={typeof value === "number" ? handleChangeText : onChange} // Handle text change and convert to number
          {...props}
        />
...
type FormPhoneInputProps<T extends React.ElementType<any>, U> = Omit<
  React.ComponentPropsWithoutRef<T>,
  "value" | "onChangeText"
> & {
  value: string;
  ...
  label?: string;
  description?: string;
  allowInternational?: boolean; // Option to allow international numbers with "+"
};

// FormPhoneInput component
const FormPhoneInput = React.forwardRef<
  React.ElementRef<typeof Input>,
  FormPhoneInputProps<typeof Input, string>
>(
  (
    {
      label,
    ...
    const inputRef = React.useRef<React.ComponentRef<typeof Input>>(null);
    // Phone number validation regex (digits, spaces, + sign, parentheses)
    ...
    const formatPhoneNumber = (text: string) => {
      ...
      // Handle different lengths for formatting
      if (cleaned.length <= 3) {
        return cleaned; // (123)
      } else if (cleaned.length <= 6) {
        return `(${cleaned.slice(0, 3)}) ${cleaned.slice(3)}`; // (123) 456
      } else if (cleaned.length <= 10) {
        return `(${cleaned.slice(0, 3)}) ${cleaned.slice(3, 6)}-${cleaned.slice(6)}`; // (123) 456-7890
      } else {
        // For longer numbers, format them differently (e.g., international numbers)
        return `+${cleaned}`;
      }
    };
    ...
    const handleTextChange = (text: string) => {
      // Determine the allowed characters (digits and optional + for international)
      const regex = allowInternational ? phoneRegex : /^[\d\s()-]*$/;

      // If the input matches the allowed characters, trigger onChange
      if (regex.test(text)) {
        const formattedPhone = formatPhoneNumber(text); // Apply formatting
        onChange?.(formattedPhone); // Call onChange with the formatted number
      }
    };
    ...
    return (
      <FormItem>
        ...
        <Input
          ...
          className={cn(
            "h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-base text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-ring focus:ring-offset-2",
...
const FormDatePicker = React.forwardRef<
  ...
  FormItemProps<typeof Calendar, Date>
>(({ label, description, value, onChange, ...props }, ref) => {
  const {
    error,
    formItemNativeID,
...
const FormSelect = React.forwardRef<
  ...
  Omit<
    FormItemProps<typeof Select, Partial<Option> | string>,
    "open" | "onOpenChange" | "onValueChange"
  >
>(({ label, description, onChange, value, returnType, ...props }, ref) => {
    ...
    <FormItem>
      ...
      <Select
        ...
        value={
          value
            ? {
                label: typeof value !== "string" ? (value?.label ?? "") : value,
